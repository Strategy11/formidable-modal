 ...
 */
class test_FrmBtsModDirectFileAccess extends FrmUnitTest {
	...
	private function all_php_file_paths( $dir = false, &$results = array() ) {
		...
		foreach ( $files as $key => $value ) {
			...
			if ( ! is_dir( $path ) ) {
				...
				if ( in_array( $value, array( 'tests', 'vendor', 'languages', 'node_modules', 'js', 'lib', 'blocks-src', 'blocks' ), true ) ) {
					continue;
				}
